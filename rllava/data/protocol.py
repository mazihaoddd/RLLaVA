...
from rllava.utils.py_functional import union_two_dict
...
class DataProtoFuture:
    ...
    collect_fn: Callable
    futures: List[any]  # 移除ray.ObjectRef依赖，使用any类型
    dispatch_fn: Callable = None

    @staticmethod
    def concat(data: List[any]) -> "DataProtoFuture":
    ...
    def get(self):
        # 移除ray.get调用，直接使用futures
        outputs = self.futures  # 直接使用，不再调用ray.get
