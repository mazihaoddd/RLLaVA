...
# UFT key parameters:
# - prefix_strategy=hint: enables cosine annealing hint schedule
# - hint_steps: total steps for cosine annealing (hint ratio decays from hint_max_ratio to 0)
# - hint_min_ratio/hint_max_ratio: bounds for hint ratio sampling
# - policy_loss.loss_mode=uft: uses UFT loss (vanilla PPO + SFT loss on prefix tokens)
# - sft_loss_coef: weight for SFT loss on hint/prefix tokens

HINT_STEPS=100  # number of steps for cosine annealing, 0 means no annealing (fixed max_ratio)

NAME=qwen2_5_math_1_5b_uft_drgrpo_0131

CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 --master-port 29509 -m rllava.train.pipeline.rlvr \
    ...
    rollout.n=8 \
    rollout.prefix_strategy=hint \
    rollout.hint_steps=${HINT_STEPS} \
    rollout.min_prefix_ratio=0.0 \
